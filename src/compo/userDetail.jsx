...
export default function UserDetail({ user, onClose, onSave }) {
  ...
  const [isVisible, setIsVisible] = useState(true); // ✅ 애니메이션용 상태
  ...
  const handleSave = () => {
    const updatedUser = { ...user, salary, memo, role, payType };
    onSave(updatedUser);
    setIsVisible(false); // 애니메이션 먼저
  };

  const handleClose = () => {
    setIsVisible(false); // ✅ 닫기 버튼도 먼저 애니메이션
  };
